    ...
    <va-modal v-model="show" hide-default-actions blur size="large">
        ...
        <template #content="{ ok }">
            ...
            <va-card-content class="d-flex flex-direction-column">
                ...
                <va-input type="number" v-model="data.change_rate" class="mb-4" :rules="[(v) => v != '' || `Champs requis`]"
                    label="Taux de change" 
                    @update:error="(e) => { error = e }"/>
                    <va-button @click="submit" :loading="isLoading">Valider</va-button>
            </va-card-content>
...
<script>
import { updatePairs } from '../../service/Route';
...
export default {
    ...
    data() {
        return {
            show: false,
            data: {
                change_rate: this.item.change_rate,
            },
            isLoading: false,
            error : false
        }
